...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        input_channels_down = [input_size]+down_filter_sizes[:-1]
        layers_conv_down = [nn.Conv2d(input_channels_down[i], down_filter_sizes[i], kernel_size=kernel_sizes[i], padding=conv_paddings[i]) for i in range(self.num_down_layers)]
        layers_bn_down = [nn.BatchNorm2d(down_filter_sizes[i]) for i in range(self.num_down_layers)]
        layers_pooling = [nn.MaxPool2d(pooling_kernel_sizes[i], stride=pooling_strides[i], return_indices=True) for i in range(self.num_down_layers)]
        ...
        input_channels_up = [down_filter_sizes[-1]]+up_filter_sizes[:-1]
        layers_conv_up = [nn.Conv2d(input_channels_up[i], up_filter_sizes[i], kernel_size=kernel_sizes[i], padding=conv_paddings[i]) for i in range(self.num_up_layers)]
        layers_bn_up = [nn.BatchNorm2d(up_filter_sizes[i]) for i in range(self.num_up_layers)]
        layers_unpooling = [nn.MaxUnpool2d(pooling_kernel_sizes[i], stride=pooling_strides[i]) for i in reversed(range(self.num_up_layers))]
    ...
    def forward(self, x):
        ...
        for i in range(self.num_up_layers):
            x = self.layers_unpooling[i](x, pool_indices[i])
